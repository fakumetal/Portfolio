import dose1 from "../../../assets/projects/DoseCalc/dose1.webp";
import dose2 from "../../../assets/projects/DoseCalc/dose2.webp";
import dose3 from "../../../assets/projects/DoseCalc/dose3.webp";
import dose4 from "../../../assets/projects/DoseCalc/dose4.webp";
import dose5 from "../../../assets/projects/DoseCalc/dose5.webp";

import image1VisitAR from "../../../assets/projects/VisitAR/1.webp";
import image3VisitAR from "../../../assets/projects/VisitAR/3.webp";
import image4VisitAR from "../../../assets/projects/VisitAR/4.webp";
import image5VisitAR from "../../../assets/projects/VisitAR/5.webp";
import image6VisitAR from "../../../assets/projects/VisitAR/6.webp";
import image7VisitAR from "../../../assets/projects/VisitAR/7.webp";
import image8VisitAR from "../../../assets/projects/VisitAR/8.webp";
import backgroundVisitAR from "../../../assets/projects/VisitAR/9.webp";

import image1Sucur from "../../../assets/projects/SucurApp/1.webp";
import image2Sucur from "../../../assets/projects/SucurApp/2.webp";
import image3Sucur from "../../../assets/projects/SucurApp/3.webp";
import image4Sucur from "../../../assets/projects/SucurApp/4.webp";
import image5Sucur from "../../../assets/projects/SucurApp/m1.webp";
import image6Sucur from "../../../assets/projects/SucurApp/m2.webp";
import image7Sucur from "../../../assets/projects/SucurApp/5.webp";
import image8Sucur from "../../../assets/projects/SucurApp/6.webp";
import backgroundSucur from "../../../assets/projects/SucurApp/0.webp";

import image1Lava from "../../../assets/projects/LavaRiso/1.webp";
import image2Lava from "../../../assets/projects/LavaRiso/2.webp";
import image3Lava from "../../../assets/projects/LavaRiso/3.webp";
import image5Lava from "../../../assets/projects/LavaRiso/5.webp";
import image6Lava from "../../../assets/projects/LavaRiso/m1.webp";
import image7Lava from "../../../assets/projects/LavaRiso/m2.webp";
import image8Lava from "../../../assets/projects/LavaRiso/m3.webp";
import image9Lava from "../../../assets/projects/LavaRiso/m4.webp";
import image10Lava from "../../../assets/projects/LavaRiso/6.webp";
import image11Lava from "../../../assets/projects/LavaRiso/7.webp";
import backgroundLava from "../../../assets/projects/LavaRiso/1.webp";

/* import backgroundEnigmax from "../../../assets/projects/Enigmax/enigmax.webp";
import backgroundCalu from "../../../assets/projects/Calu/calu.webp";
import image1CRM from "../../../assets/projects/CRM/1.webp";
import image2CRM from "../../../assets/projects/CRM/2.webp";
import image3CRM from "../../../assets/projects/CRM/3.webp";
import image4CRM from "../../../assets/projects/CRM/4.webp"; */

import eyesHappy from "../../../assets/projects/eyes-happy.webp";

import bastion from "../../../assets/projects/bastion.ico";
import kiru from "../../../assets/projects/Kiru/kiru-logo.png";
import altocenter from "../../../assets/projects/AltoCenter/altocenter.png";
/* 
import image1Ec from "../../../assets/projects/ecommerce/1.webp";
import image2Ec from "../../../assets/projects/ecommerce/2.webp";
import image3Ec from "../../../assets/projects/ecommerce/3.webp";
import image4Ec from "../../../assets/projects/ecommerce/4.webp";
import image5Ec from "../../../assets/projects/ecommerce/5.webp";
import image6Ec from "../../../assets/projects/ecommerce/6.webp";
import image7Ec from "../../../assets/projects/ecommerce/7.webp";
import image8Ec from "../../../assets/projects/ecommerce/8.webp";
import image9Ec from "../../../assets/projects/ecommerce/9.webp"; */
export const projectsData = [
 /*   {
    title: "Ecommerce",
    backgroundImage: image1Ec,
    description: "Ecommerce responsivo para la venta de ropa interior femenina. Utilicé React para el frontend y Firebase para la autenticación, gestión de datos, almacenamiento y Cloudinary las imagenes ofreciendo una alternativa totalmente gratuita de servidor dado los bajos requisitos actuales. La plataforma está diseñada para ofrecer una experiencia de compra intuitiva y segura, ademas de tener un panel de administrador para la carga/edición de productos, reservas y ventas. ",
    images: [image1Ec, image2Ec, image3Ec, image5Ec, image6Ec, image7Ec, image8Ec, image9Ec],
    isInDevelopment: true,
  }, */
 {
  title: "KIRU ",
  backgroundImage: kiru,
  description:
   " KIRU es una landing especializada en urgencias odontológicas que ofrece atención rápida y efectiva. Además, integra una variedad de servicios médicos complementarios como psicología, cardiología y medicina clínica, brindando una solución integral de salud en un solo lugar. Ideal para quienes buscan atención inmediata y multidisciplinaria. Realizado con NextJs  Explora más en <a href='https://urgenciasodontologicas.netlify.app/' target='_blank' rel='noopener noreferrer'>KIRU - Urgencias Odontológicas</a>.",
  images: [ ],
  isInDevelopment: true,
 },
 {
  title: "Bastion Alquileres",
  backgroundImage: bastion,
  description:
   "Landing page diseñada para gestionar el alquiler de alojamientos en Villa La Angostura. Incluye un calendario interactivo para visualizar y realizar reservas, además de la integración con MercadoPago Checkout Pro para garantizar pagos seguros. El proyecto fue realizado con Next.js y Firebase. Explora más en <a href='https://bastionalquileres.com/' target='_blank' rel='noopener noreferrer'>Bastion Alquileres</a>.",
  images: [bastion],
  isInDevelopment: false,
 },
 {
  title: "LavaRiso",
  backgroundImage: backgroundLava,
  description:
   "Aplicación CRM/ERP desarrollada con Ionic y Electron para empresas de reparación de electrodomésticos. Incluye funcionalidades contables, control de stock, y una app móvil para la gestión de técnicos en campo. Proyecto realizado en <a href='https://pochitamadev.com/' target='_blank' rel='noopener noreferrer'>PochitamaDev</a>",
  images: [image1Lava, image2Lava, image3Lava, image5Lava, image6Lava, image7Lava, image8Lava, image9Lava, image10Lava, image11Lava],
  isInDevelopment: false,
 },
 {
  title: "DoseCalc",
  backgroundImage: dose1,
  description:
   "DoseCalc es una aplicación simple desarrollada en React Native diseñada específicamente para enfermería, facilitando el cálculo preciso de dosis de medicamentos inyectables. Con una interfaz intuitiva permite a los profesionales de la salud ingresar concentraciones de medicamentos y calcular las dosis según la presentación. Ademas posee un simple pero útil conversor de unidades.",
  images: [dose1, dose2, dose3, dose4, dose5],
  isInDevelopment: false,
 },
 {
  title: "Portfolio",
  backgroundImage: eyesHappy,
  description:
   "Este portafolio ha evolucionado infiltrandose en la lista de proyectos. Si no se detiene ahora, pronto dominará el mundo!. Para luchar contra lo inevitable mejor elimínalo antes de que sea tarde.",
  url: "#",
  images: [eyesHappy],
  isInDevelopment: false,
 },
   {
    title: "Alto Center",
    backgroundImage: altocenter,
    description: "  Alto Center es un complejo hotelero moderno con múltiples servicios como spa, gimnasio y salones para eventos. La landing fue desarrollada con Next.js, TypeScript e imágenes alojadas en Cloudinary. Visita  <a href='https://altocenter.netlify.app/' target='_blank' rel='noopener noreferrer'>AltoCenter</a>",
    images: [ ],
    isInDevelopment: true,
  },
 /*   {
    title: "CRM",
    backgroundImage: image1CRM,
    description: "Plataforma para estudios de mercado inmobiliario, diseñada para proporcionar análisis detallados y datos clave del sector. Incluye un visor de PDFs que permite acceder a informes y estudios de mercado directamente desde la plataforma, mejorando la comprensión de tendencias y oportunidades. Proyecto realizado en <a href='https://pochitamadev.com/' target='_blank' rel='noopener noreferrer'>PochitamaDev</a>",
    images: [image1CRM, image2CRM, image3CRM, image4CRM],
    isInDevelopment: true,
  }, */

 {
  title: "VisitAR",
  backgroundImage: backgroundVisitAR,
  description:
   "Aplicación desarrollada en Ionic para gestionar y hacer seguimiento de visitas de pacientes en domicilio. Permite a los profesionales de la salud registrar y actualizar información en tiempo real, utilizando Firebase para la autenticación y sincronización.",
  images: [image8VisitAR, image1VisitAR, image3VisitAR, image4VisitAR, image5VisitAR, image6VisitAR, image7VisitAR],
  isInDevelopment: false,
 },

 /*  
  {
    title: "Enigmax",
    backgroundImage: backgroundEnigmax,
    description: "Empresa dedicada a la realización de juegos de rol inmersivos e innovadores.Frontend: React, Backend: Node.js y MySQL . Visita <a href='https://www.enigmaxoficial.com/#/' target='_blank' rel='noopener noreferrer'>EnigmaxOficial</a>. Proyecto realizado en <a href='https://pochitamadev.com/' target='_blank' rel='noopener noreferrer'>PochitamaDev</a>",
    images: [backgroundEnigmax],
    isInDevelopment: false,
  }, */

 {
  title: "SucurApp",
  backgroundImage: backgroundSucur,
  description:
   "Solución contable avanzada desarrollada con Ionic ( App )  y Electron ( Software ) para monitorizar ingresos y egresos en sucursales mayoristas. Automatiza procesos financieros, facilita el intercambio con Excel y optimiza la gestión financiera. Backend: AWS Lightsail + RDS. Proyecto realizado en <a href='https://pochitamadev.com/' target='_blank' rel='noopener noreferrer'>PochitamaDev</a>",
  images: [image1Sucur, image2Sucur, image3Sucur, image4Sucur, image5Sucur, image6Sucur, image7Sucur, image8Sucur],
  isInDevelopment: false,
 },
 // {
 //   title: "Calu",
 //   backgroundImage: backgroundCalu,
 //   description: "Sitio web para una agencia de marketing con diseño atractivo y plataforma back-end personalizable. Permite la edición detallada, gestión de contenidos, visualización de analíticas y optimización SEO. Visita <a href='https://calu-mkt.com/#/' target='_blank' rel='noopener noreferrer'>calu-mkt</a>. Proyecto realizado en <a href='https://pochitamadev.com/' target='_blank' rel='noopener noreferrer'>PochitamaDev</a>",
 //   images: [backgroundCalu],
 //   isInDevelopment: false,
 // },
];
